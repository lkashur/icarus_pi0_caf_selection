 ...
 * MC
 */
std::ofstream output("output_mc_numi_test_mu100.log"); // analysis
...
void write_pair(const caf::SRSpillProxy* sr, const caf::SRInteractionTruthDLPProxy& i, const caf::SRInteractionDLPProxy& j)
{
  output  << CSV(sr->hdr.run) << CSV(sr->hdr.evt) << CSV(sr->hdr.subrun) // USE FOR EVERYTHING ELSE
    ...
    //<< CSV(std::string(sr->hdr.sourceName))
	  ...
	  << CSV(cuts::flash_cut_numi(j))
	  << CSV(cuts::fiducial_cut(j))
	  << CSV(cuts::topological_1mu2gamma_cut(j))
	  << CSV(cuts::muon_containment_cut(j))
	  << CSV(cuts::all_1mu2gamma_numi_cut(j))
	  << std::endl;
}
...
const SpillMultiVar kDataInfo([](const caf::SRSpillProxy* sr)
{
  for(auto const & i : sr->dlp)
    {
      if(cuts::all_1mu2gamma_numi_cut(i))
	{
	  OUT(output,"DATA");
	  write_reco(sr, i);
	}
	  ...
	  if(category == 0)
	      ...
	      if(cuts::matched(i))
		{
		  ...
		  output << CSV(std::string(sr->hdr.sourceName))
			 ...
			 << CSV(vars::pi0_costheta(i))
			 << CSV(vars::pi0_mass(i))
			 << CSV(cuts::all_1mu2gamma_numi_cut(r))
			 << std::endl;
