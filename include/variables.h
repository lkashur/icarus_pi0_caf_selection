...
namespace vars
{
    ...
    template<class T>
      double category(const T & interaction)
      {
        ...
        if(cuts::signal_1mu1pi0(interaction))
          {
            if(cuts::fiducial_muon_containment_cut(interaction)) cat = 0;
            else cat = 1;
          }
        // Neutrino Background                         
    ...
    */
	...
	// 1mu1pi0 signal
	if(cuts::signal_1mu1pi0(interaction) && cuts::fiducial_muon_containment_cut(interaction))
	  {
	    // Loop over particles
	    for(size_t i(0); i < interaction.particles.size(); ++i)
	      {
	...
	if(cuts::all_1mu2gamma_numi_cut(interaction)) // REMEMBER TO CHANGE BETWEEN DATA AND MC
	  {
	    // Temp. storage
	    vector< pair<size_t, double> > photon_energies;
