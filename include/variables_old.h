...
namespace vars
{
    ...
    */
    ...
    template<class T>
      double category(const T & interaction)
      {
        ...
        if(cuts::signal_1mu0pi1pi0(interaction))
          {
            if(cuts::fiducial_track_containment_cut(interaction)) cat = 0;
            else cat = 1;
          }
        // Neutrino Background                         
	...
	if(interaction.nu_id > -1)
	  {
	    ...
	    int primary_muon_at_count(0);
	    int primary_pion_count(0);
	    int primary_pion_at_count(0);
	    int primary_pi0_count(0);
	    int primary_pi0_at_count(0);
		...
		if(p.pid == 2 && p.is_primary)
		  {
		    primary_muon_count++;
		    if(p.ke >= MIN_MUON_ENERGY) primary_muon_at_count++;
		  }
		// Primary pion count
		if(p.pid == 3 && p.is_primary)
		  {
		    primary_pion_count++;
		    if(p.ke >= MIN_PION_ENERGY) primary_pion_at_count++;
		  }
		// Group primary pi0 photons
		if(p.pid == 0 && p.is_primary && p.parent_pdg_code == 111)
                  {
		...
		int num_primary_photon_daughters_at = 0;
                for(auto e : pi0.second)
                  { 
		    num_primary_photon_daughters++;
                    if(e > MIN_PHOTON_ENERGY) num_primary_photon_daughters_at++;
                  }
		...
		// Only above-threshold primary photons
		if(num_primary_photon_daughters_at == 2)
		  {
		    primary_pi0_at_count++;
		  }
	      }
	    ...
	    if(primary_muon_count == 1 && primary_pion_count == 0 && primary_pi0_count == 1 && nonprimary_pi0_count == 0 && interaction.current_type == 0)
	      {
		// Meets all signal criteria
		if(primary_muon_at_count == 1 && primary_pion_at_count == 0 && primary_pi0_at_count == 1 && cuts::fiducial_track_containment_cut(interaction))
		  {
		    cat = 0;
		  }
		// Fails some part of signal criteria
		else
		  {
		    cat = 1;
		  }
	    ...
	    // 1mu1pi0 + Npi (N > 0)
	    else if(primary_muon_at_count == 1 && primary_pion_at_count > 0 && primary_pi0_at_count == 1 && nonprimary_pi0_count == 0 && interaction.current_type == 0)
	      {
		cat = 2;
	      }
	    // 1muNpi0 (N > 1)
	    else if(primary_muon_at_count == 1 && primary_pi0_at_count > 1 && interaction.current_type == 0)
	      {
		cat = 3;
	      }
	    // NC pi0
	    else if(primary_muon_at_count == 0 && primary_pi0_at_count > 0 && interaction.current_type == 1)
	      {
		cat = 4;
	      }
	    // Other nu
    ...
    template<class T>
      double category_topology_old(const T & interaction)
      {
        // Cosmic                              
        uint16_t cat(5);
        if(interaction.nu_id > -1)
		...
		//cout << "hello" << endl;
		//cout << parent_creation_process << endl;
		//cout << string(p.parent_creation_process) << endl;
		//cout << p.is_contained << endl;
                if(p.pid == 0 && !p.is_primary && p.parent_pdg_code == 111 && abs(p.ancestor_pdg_code) == 211)
                  {
		...
		cex_pi0_count++;
            ...
            // Categorization
	    if(primary_muon_count == 1 && primary_pi0_count == 1 && cex_pi0_count == 0 && interaction.current_type == 0 && cuts::fiducial_muon_containment_cut(interaction))
	      {
		cat = 0;
		/*
		// 1mu1pi0, contained and fiducial
		if (cuts::muon_containment_cut(interaction) && interaction.is_fiducial)
		  {
		    cat = 0;
		  }
		// 1mu1pi0, either not contained or not fiducial
		else
		  {
		    cat = 1; // Change to 1 if you want to separate out contained/non-contained
		  }
		*/
	      }
	    // 1muNpi0
	    else if(primary_muon_count == 1 && primary_pi0_count > 1 && cex_pi0_count == 0 && interaction.current_type == 0)
	      {
		cat = 1;
	      }
	    // 1muCex
	    else if(primary_muon_count == 1 && cex_pi0_count > 0 && interaction.current_type == 0)
	      {
		cat = 2;
	      }
	    // NC
    ...
    */
	...
	// 1mu1pi0 signal
	if(cuts::signal_1mu0pi1pi0(interaction) && cuts::fiducial_muon_containment_cut(interaction))
	  {
	    // Loop over particles
	    for(size_t i(0); i < interaction.particles.size(); ++i)
	      {
